...
class PlayerPageBase extends Component {
  ...
  stopPlaying() {
    ...
    this.props.firebase.stopPlaying(gameId, role, uid)
      .then(() => {
  ...
  render() {
    const { role, gameId } = this.props.match.params;
    const uid = this.props.authUser.uid;
    const { game, currentGame, activeQuestion } = this.props;
    const { gameStatus, host, p1, p2, p3 } = game;

    const isMyGame = currentGame && currentGame.gameId == gameId && ['p1', 'p2', 'p3'].indexOf(currentGame.role) != -1 && currentGame.role == role;
    if (!isMyGame) {
      this.props.history.push(ROUTES.HOME);
      return null;
    }
    ...
    if (!gameStatus) {
      ...
      waitingMsg = "Waiting for others to join...";
    }
    ...
    return (
      <div>
        ...
        {!errorMsg && <StopPlayingButton onClick={this.stopPlaying} />}
        {!errorMsg && !waitingMsg && game.gameStatus == 'Active' && <ActivePlayerScreen gameId={game.gameId} role={role} activeQuestion={activeQuestion}/>}
